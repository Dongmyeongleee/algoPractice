...
for k in range(N):
    for i in range(N):
        for j in range(N):
            if arr[i][k] == 1 and arr[k][j] == 1:   # 두개가 연결이 되어있으면
                arr[i][j] = 1   # 나머지도 연결되어 있음 플로이드와샬 점화식 이용

# for k in range(N):
#     for i in range(N):
#         for j in range(N):
#             if arr[i][j] == 1 and arr[k][j] == 1:   # 두개가 연결이 되어있으면
#                 arr[i][k] = 1   # 나머지도 연결되어 있음 플로이드와샬 점화식 이용
